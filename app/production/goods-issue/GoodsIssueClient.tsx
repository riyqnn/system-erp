...
import { Plus, X, Warning, MagnifyingGlass, Package } from '@phosphor-icons/react'
...
type FormState = { production_order_id: string; product_id: string; quantity: string; issue_date: string; status: string; notes: string }
const emptyForm: FormState = { production_order_id: '', product_id: '', quantity: '', issue_date: new Date().toISOString().slice(0, 10), status: 'issued', notes: '' }
...
export function GoodsIssueClient() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [giRes, ordRes, prodRes] = await Promise.all([
  ...
  async function handleSave() {
    setFormError(null)
    if (!form.quantity || Number(form.quantity) <= 0) { setFormError('Quantity harus > 0'); return }
    setSaving(true)
    try {
      const res = await fetch('/api/production/good-issue', {
        method: 'POST',
      ...
      <div className="space-y-6 max-w-[1400px] mx-auto">
        <div className="flex items-end justify-between">
          <div>
            ...
            <ModuleHeader title="Goods Issue" />
            <p className="text-slate-500 -mt-4 text-sm">
              {rows.length} records ·{' '}
              <span className="text-green-600 font-medium">{rows.filter(r => r.status === 'issued').length} issued</span>
            </p>
          </div>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50/80 text-slate-500 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-3.5 font-medium">Issue Date</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {loading ? (
                    ...
                    <tr key={r.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 font-medium text-slate-900">{r.products?.name ?? '—'}</td>
                      <td className="px-6 py-4 text-right tabular-nums text-slate-900">{r.quantity.toLocaleString()}</td>
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Quantity">
                    <Input type="number" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} placeholder="e.g. 500" className="h-10 border-slate-200" />
                  </Field>
