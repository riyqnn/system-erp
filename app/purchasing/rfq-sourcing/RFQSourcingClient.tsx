...
export function RFQSourcingClient() {
  ...
  const fetchRFQData = async () => {
    try {
      ...
      if (prNoFromUrl && prResponse.ok) {
        // sorted eligible PR newest first
        const prList: PurchaseRequisition[] = (prResult.data || []).map((item: AnyObject) => normalizePR(item))
        prList.sort((a, b) => {
          const dateA = new Date(a.requestDate || a.createdAt || 0).getTime()
          const dateB = new Date(b.requestDate || b.createdAt || 0).getTime()
          return dateB - dateA
        })
        const eligiblePRList = prList.filter(pr => pr.status === 'PROCESSED')

        const selectedPR = eligiblePRList.find(
          (item) =>
        ...
        // Dropdown: allow to select any eligible PR to start new RFQ
        if (!prNoFromUrl && eligiblePRList.length > 0) {
          const draftRFQ = buildRFQFromPR(eligiblePRList[0], suppliers)
          if (draftRFQ) {
            combinedRFQ = [draftRFQ, ...databaseRFQ]
            selectedId = draftRFQ.id
          }
  ...
  const handleSendRFQ = async () => {
    ...
    try {
      ...
      router.push('/purchasing/negotiation')
    } catch (error) {
      setErrorMessage(
