...
export function ThreeWayMatchingClient() {
  ...
  const handleSendToFinance = async () => {
    ...
    if (selectedMatching.sentToFinance) {
      setErrorMessage('This matching document has already been sent to Finance.')
      return
    }

    const confirmed = window.confirm(
      'Run Three-Way Matching for this document?'
    )
            ...
            <div className="rounded-xl border border-red-100 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 flex flex-col justify-end gap-3 sm:flex-row">
                ...
                <button
                  ...
                  disabled={isSending || selectedMatching.sentToFinance}
                ...
                {selectedMatching.matchStatus === 'PRICE_MISMATCH' && (
                  <button
                    type="button"
                    onClick={async () => {
                      if (!confirm('Request correction for this invoice? This will remove the current invoice data so a revised one can be inputted.')) return;
                      try {
                        const res = await fetch(`/api/purchasing/three-way-matchings?poId=${selectedMatching.poNo}`, { method: 'DELETE' });
                        if (res.ok) {
                          alert('Correction requested! The invalid invoice has been cleared.');
                          fetchMatchings();
                        } else {
                    ...
                    className="rounded-lg bg-green-600 px-5 py-2 text-sm font-medium text-white hover:bg-green-700 ml-auto"
                  >
                    Request Correction
                  </button>
                )}
