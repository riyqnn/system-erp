...
type RawPRData = {
  ...
  isBudgetProvided?: boolean
  is_budget_provided?: boolean
  items?: RawPRItemData[]
  purchasing_purchase_requisition_items?: RawPRItemData[]
  purchase_requisition_items?: RawPRItemData[]
}
...
function normalizePR(raw: RawPRData): PurchaseRequisition {
  const rawItems =
    raw.items ||
  ...
  const items: PurchaseRequisitionItem[] = rawItems.map((item: RawPRItemData) => {
  ...
  return {
    ...
    requesterName:
      ...
      raw.requester_name ||
      raw.requestedBy ||
      (raw.requested_by ? String(raw.requested_by) : undefined) ||
      '-',
    requestedById: raw.requestedById || (raw.requested_by ? Number(raw.requested_by) : null) || null,
    ...
    statusLabel: raw.statusLabel || getStatusLabel(raw.status || 'PENDING'),
...
export function PurchaseRequisitionClient() {
  ...
  const fetchPurchaseRequisitions = async () => {
    try {
      ...
      const normalizedData = (result.data || []).map((item: RawPRData) =>
        normalizePR(item)
      )
  ...
  const pendingReviewCount = purchaseRequisitions.filter(
    (item) => isPendingReview(item.status)
  ).length
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {canProcessRFQ && isReadyForRFQ(item.status) && (
                              <Link
                                href={`/purchasing/rfq-sourcing?prNo=${encodeURIComponent(
                                  item.prNo
                                )}`}
